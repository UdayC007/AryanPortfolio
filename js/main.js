    ...
    function renderProjects() {
        ...
        projects.forEach((p, i) => {
            ...
            const thumb = p.image
                ...
                : `<div class="project-thumb-placeholder" style="color:${esc(p.color||'#00d4ff')}">${esc(p.title.charAt(0))}</div>`;
            const tags = (p.tags||[]).map(t => `<span class="project-tag">${esc(t)}</span>`).join('');
            const link = p.link ? `<a href="${esc(p.link)}" target="_blank" class="project-link">View →</a>` : '';
